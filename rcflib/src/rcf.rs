...
use crate::{
    ...
    samplerplustree::{nodeview::UpdatableNodeView, samplerplustree::SamplerPlusTree},
...
pub fn create_rcf(
    ...
    RCFBuilder::<u64, u64>::new(dimensions / shingle_size, shingle_size)
        ...
        .build_default()
        .unwrap() //unwrap kept for deprecated function
}

impl<C, L, P, N, Label, Attributes> AugmentedRCF<Label, Attributes>
    for RCFStruct<C, L, P, N, Label, Attributes>
where
    C: Location,
    usize: From<C>,
    L: Location,
    ...
    <N as TryFrom<usize>>::Error: Debug,
{
    ...
    fn is_output_ready(&self) -> bool {
        ((self.output_after
            + if self.is_internal_shingling_enabled() {
                self.shingle_size - 1
            } else {
                0
            }) as u64)
    ...
    fn size(&self) -> usize {
        ...
        sum + self.point_store.size()
            + std::mem::size_of::<RCFStruct<C, L, P, N, Label, Attributes>>()
    }
    ...
    RCFBuilder<Label, Attributes>
{
    ...
    pub fn build_tiny<Update: Send + Sync + Copy, Operate: Send + Sync + Copy + Eq + Hash>(
        ...
        check_argument(
            dimensions < (u8::MAX as usize) && (self.rcf_options.capacity - 1 <= u8::MAX as usize),
            "incorrect parameters",
        )?;
        check_argument(
            self.rcf_options.capacity * (1 + self.rcf_options.number_of_trees) * self.shingle_size
                <= u16::MAX as usize,
            " incorrect parameters",
        )?;
    ...
    pub fn build_medium<Update: Send + Sync + Copy, Operate: Send + Sync + Copy + Eq + Hash>(
        ...
        let random_seed = self
            ...
            .unwrap_or(rand::random::<u64>());
        check_argument(
            (dimensions < u16::MAX as usize)
                && (self.rcf_options.capacity - 1 <= u16::MAX as usize),
            " incorrect parameters",
        )?;
    ...
    RCFOptions<Label, Attributes>
{
    pub fn validate(&self) -> Result<()> {
        ...
        check_argument(
            self.time_decay.unwrap_or(0.0) >= 0.0,
            "time decay cannot be negative",
        )?;
        check_argument(
            self.bounding_box_cache_fraction >= 0.0 && self.bounding_box_cache_fraction <= 1.0,
            "bounding box cache fraction is in [0,1]",
        )?;
        check_argument(
            !self.propagate_attributes || self.store_attributes,
