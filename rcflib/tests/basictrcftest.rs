...
use rcflib::trcf::types::TransformMethod::{
    DIFFERENCE, NORMALIZE, NORMALIZE_DIFFERENCE, SUBTRACT_MA, WEIGHTED,
};
use rcflib::{
    common::multidimdatawithkey, trcf::basictrcf::BasicTRCF, trcf::types::TransformMethod::NONE,
};
#[cfg(test)]
...
fn basic_trcf(transform_method: TransformMethod) {
    ...
    for i in 0..data_with_key.data.len() {
            ...
            && data_with_key.change_indices[next_index] == i
        {
            print!(
                "timestamp {} INJECT [ {}",
                i, data_with_key.changes[next_index][0]
            );
...
fn trcf_scale(
    ...
    for i in 0..data_with_key.data.len() {
        ...
        if result.anomaly_grade > 0.0 {
            ...
            potential_anomalies
                .push((i as i32 + result.last_anomaly.as_ref().unwrap().relative_index) as usize);
        }
    }

    println!(
        "{} anomalies injected in {} points",
        data_with_key.changes.len(),
        data_size
    );
    let mut common = 0;
    let mut i: usize = 0;
