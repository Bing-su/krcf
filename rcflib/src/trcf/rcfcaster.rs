...
impl RCFCaster {
    pub fn process(&mut self, point: &[f32], timestamp: u64) -> Result<Descriptor> {
        ...
        if self.rcf.is_output_ready() {
            self.error_handler.update_actuals(
                &result.current_input,
                result.deviations_post.as_ref().expect("should be present"),
            )?;
    ...
    pub fn extrapolate(
        ...
        self.error_handler
            .calibrate(self.calibration_method, &mut a.0)?;
        Ok(a)
    }
...
impl RCFCasterBuilder {
    ...
    pub fn build(&self) -> Result<RCFCaster> {
        ...
        let error_horizon = self
            .rcf_caster_options
            .error_horizon
            .unwrap_or(MAX_ERROR_HORIZON);
        check_argument(
            error_horizon <= MAX_ERROR_HORIZON,
            "calibration horizon should be smaller",
        )?;
        let error_handler = ErrorHandler::new(
            self.rcf_caster_options.add_error,
            self.input_dimensions,
            self.rcf_caster_options.forecast_horizon,
            error_horizon,
            DEFAULT_ERROR_PERCENTILE,
        );
        Ok(RCFCaster {
