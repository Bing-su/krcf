...
>: BasicStore
{
    ...
    fn recompute_attribute_vec<PS: PointStore<Label, Attributes>>(
        &mut self,
        index: usize,
        point_store: &PS,
    ) -> Result<()>;
    fn pointsum<PS: PointStore<Label, Attributes>>(
        &self,
        index: usize,
        point_store: &PS,
    ) -> Result<Vec<f32>>;
    fn recompute_pointsum<PS: PointStore<Label, Attributes>>(
        &mut self,
        index: usize,
        point_store: &PS,
    ) -> Result<()>;
    ...
    fn reconstruct_box<PS: PointStore<Label, Attributes>>(
        &self,
        index: usize,
        point_store: &PS,
    ) -> Result<BoundingBox>;
    ...
    fn bounding_box<PS: PointStore<Label, Attributes>>(
        &self,
        index: usize,
        point_store: &PS,
    ) -> Result<BoundingBox>;
    ...
    <N as TryFrom<usize>>::Error: Debug,
{
    ...
    pub fn invalidate_pointsum(&mut self, index: usize) -> Result<()> {
        ...
        for x in
            self.pointsum[(index * self.dimensions)..((index + 1) * self.dimensions)].iter_mut()
        {
            *x = 0.0;
        }
        Ok(())
    ...
    pub fn increase_leaf_mass(&mut self, index: usize) -> Result<()> {
        ...
        Err(RCFError::InvalidArgument {
            msg: " incorrect call with a non-leaf index",
        })
    }
    ...
    pub fn delete_internal_node(&mut self, index: usize) -> Result<()> {
        ...
        if self.propagate_attributes {
            self.propagated_attributes
                .remove(&index.try_into().expect("incorrect state"));
        }
    ...
    Attributes: Sync + Copy + Hash + Eq + Send,
{
    fn attribut_vec<PS: PointStore<Label, Attributes>>(
        ...
        if self.is_leaf(index) {
            let list = self
                .attributes
                .get(&(self.leaf_point_index(index)?.try_into().unwrap()))
    ...
    fn recompute_attribute_vec<PS: PointStore<Label, Attributes>>(
        ...
        self.propagated_attributes
            .insert(index.try_into().expect("incorrect state"), left);
        Ok(())
    }
    ...
    fn pointsum<PS: PointStore<Label, Attributes>>(
        ...
        if self.is_leaf(index) {
            ...
            Ok(Vec::from(
                &self.pointsum[(index * self.dimensions)..((index + 1) * self.dimensions)],
            ))
        }
    }
    ...
    fn recompute_pointsum<PS: PointStore<Label, Attributes>>(
        ...
        for ((x, y), z) in self.pointsum[(index * self.dimensions)..((index + 1) * self.dimensions)]
            .iter_mut()
            .zip(left)
            .zip(right)
        {
    ...
    fn modify_in_place_probability_of_cut_di_vector<PS: PointStore<Label, Attributes>>(
        ...
        check_argument(
            di_vector.high.len() == point.len(),
            " incorrect dimensions of bounding box",
        )?;
        let node_idx: usize = self.translate(index);
        if node_idx != usize::MAX {
            let base = 2 * node_idx * self.dimensions;
    ...
    fn grow_node_box<PS: PointStore<Label, Attributes>>(
        ...
        if self.is_leaf(sibling) {
            ...
            if idx != usize::MAX {
                ...
                self.grow_node_box(bounding_box, point_store, sibling, self.left_index(sibling))?;
                self.grow_node_box(
                    bounding_box,
                    point_store,
                    sibling,
    ...
    fn check_left<PS: PointStore<Label, Attributes>>(
        ...
        Ok(
            self.check_left(self.left_index(index), dim, value, point_store)?
    ...
    fn check_right<PS: PointStore<Label, Attributes>>(
        ...
        Ok(
            self.check_right(self.left_index(index), dim, value, point_store)?
    ...
    <N as TryFrom<usize>>::Error: Debug,
{
    ...
    fn attribute_at_leaf(&self, point_index: usize) -> Result<Vec<(usize, usize)>> {
        self.attributes
            .get(&point_index.try_into().expect("out of range"))
            .expect("should be present")
            .iter()
            .map(|(&x, &y)| Ok((x.into(), y.into())))
            .collect()
    }
}
