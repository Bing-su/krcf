...
impl Preprocessor {
                    ...
                    let past_values = if result.relative_index == 0 {
                        ...
                        Vec::from(
                            &self.last_shingled_input
                                [(start + base_dimension)..(start + 2 * base_dimension)],
                        )
                    };
                    if let Some(x) = &result.expected_rcf_point {
                        let expected_values_list = vec![self.transformer.invert(
                            &x[start..start + base_dimension],
                            &self.last_shingled_input[start..start + base_dimension],
                        )];
                        let likelihood_of_values = vec![1.0f32];
                        result.last_anomaly = Some(AnomalyInformation {
        ...
        U: RCF,
    {
        if self.values_seen == self.start_normalization {
            ...
            for i in 0..self.initial_values.len() {
                self.transformer
                    .update(&self.initial_values[i], &previous)?;
            ...
            for i in 0..self.input_dimensions {
                self.last_shingled_input[dimension - self.input_dimensions + i] =
                    self.initial_values[0][i];
            }
    ...
    pub fn invert_extrapolation(
        &self,
        mut range_vector: RangeVector<f32>,
    ) -> Result<(RangeVector<f32>, Option<RangeVector<f64>>)> {
        if self.forest_mode != TIME_AUGMENTED {
            let dimension = self.input_dimensions * self.shingle_size;
            self.transformer.invert_forecast(
                &mut range_vector,
                &self.last_shingled_input[(dimension - self.input_dimensions)..dimension],
            )?;
            ...
            let dimension = self.input_dimensions * self.shingle_size;
            let mut sub_range_vector: RangeVector<f32> =
                RangeVector::<f32>::new(lookahead * dimension);
            let mut time_range_vector: RangeVector<f64> = RangeVector::<f64>::new(lookahead);
            for i in 0..lookahead {
                for j in 0..dimension {
                    sub_range_vector.upper[i * dimension + j] =
                        range_vector.upper[i * augmented + j];
                    sub_range_vector.values[i * dimension + j] =
                        range_vector.values[i * augmented + j];
                    sub_range_vector.lower[i * dimension + j] =
                        range_vector.lower[i * augmented + j];
                }
                time_range_vector.upper[i] =
                    self.invert_time(range_vector.upper[i * augmented + dimension]);
                time_range_vector.values[i] =
                    self.invert_time(range_vector.values[i * augmented + dimension]);
                time_range_vector.lower[i] =
                    self.invert_time(range_vector.lower[i * augmented + dimension]);
            ...
            self.transformer.invert_forecast(
                &mut sub_range_vector,
                &self.last_shingled_input[(dimension - self.input_dimensions)..dimension],
            )?;
            time_range_vector.cascaded_add(&vec![
                self.previous_timestamps[self.shingle_size - 1] as f64,
            ])?;
    ...
    pub fn shift(&self) -> Vec<f32> {
        if self.forest_mode != TIME_AUGMENTED {
            ...
            answer.push(
                self.previous_timestamps[self.shingle_size - 1] as f32 + self.timedrift() as f32,
            );
            answer
        }
    }
    ...
    pub fn transform(&self, input: &[f32], timestamp: u64) -> Vec<f32> {
        ...
        if self.forest_mode == TIME_AUGMENTED {
            ...
            answer.push(
                (timestamp as f64 - previous as f64 * (self.weight_time as f64) / self.timescale())
                    as f32,
            );
        }
        answer
    ...
    pub fn difference_deviations(&self) -> Vec<f32> {
        if self.forest_mode != TIME_AUGMENTED {
            ...
            answer
                .push(self.weight_time as f32 * (self.timestamp_deviations[1].deviation() as f32));
            answer
        }
    }
...
impl PreprocessorBuilder {
    ...
    pub fn build(&self) -> Result<Preprocessor> {
        ...
        check_argument(
            self.input_dimensions == weights.len(),
            "incorrect length of weights",
        )?;
        check_argument(
            transform_decay >= 0.0 && transform_decay <= 1.0,
