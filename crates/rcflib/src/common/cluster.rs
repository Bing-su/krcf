...
pub trait IntermediateCluster<Z, Q, T: ?Sized> {
    ...
    fn distance_to_cluster<'a>(
        ...
        dictionary: &'a [Z],
        get_point: fn(usize, &'a [Z]) -> &'a T,
        other: &dyn IntermediateCluster<Z, Q, T>,
        distance: fn(&T, &T) -> f64,
    ) -> (f64, usize, usize);
    ...
    fn absorb<'a>(
        ...
        dictionary: &'a [Z],
        get_point: fn(usize, &'a [Z]) -> &'a T,
        another: &dyn IntermediateCluster<Z, Q, T>,
        distance: fn(&T, &T) -> f64,
    ) -> Result<()>;
...
impl Center {
    ...
    fn recompute_rad_vec<'a, Z>(
        &mut self,
        points: &'a [Z],
        get_point: fn(usize, &'a [Z]) -> &'a Vec<f32>,
        dist: fn(&Vec<f32>, &Vec<f32>) -> f64,
    ) -> f64 {
        let old_value = self.sum_of_radii;
        self.sum_of_radii = 0.0;
        for j in 0..self.points.len() {
    ...
    fn absorb_list(
        ...
        for i in 0..dimensions {
            self.representative[i] = (factor * (self.representative[i] as f64)
                + (1.0 - factor) * (other_list[closest.1].0[i] as f64))
                as f32;
        }
...
fn process_point<'a, Z, U, Q, T: ?Sized>(
    dictionary: &'a [Z],
    get_point: fn(usize, &'a [Z]) -> &'a T,
    index: usize,
    centers: &mut [U],
    ...
    T: Sync,
{
    ...
    for j in 0..centers.len() {
        dist[j] = centers[j].distance_to_point(
            dictionary,
            get_point,
            get_point(index, dictionary),
            distance,
        );
    ...
    if samples.len() == 0 {
        for i in 0..dictionary.len() {
            process_point(
                ...
                i,
                centers,
                get_weight(i, dictionary, weights),
                distance,
            )?;
        }
    } else {
    ...
    V: Copy,
{
    ...
    check_argument(
        max_allowed > 0,
        " number of clusters has to be greater or equal to 1",
    )?;
    ...
    let sampled_sum: f32 = if dictionary.len() > approximate_bound {
        ...
        (0..dictionary.len())
            .into_iter()
            .map(|x| get_weight(x, dictionary, weights))
            .sum()
    };
    ...
    for _k in 0..10 * max_allowed {
        ...
        for i in 0..centers.len() {
            let t = centers[i].distance_to_point(
                dictionary,
                get_point,
                get_point(index, &dictionary),
                distance,
    ...
    while keep_reducing_centers {
        ...
        while lower < centers.len() - 1 && !found_merge {
            ...
            for j in lower + 1..centers.len() {
                let dist = centers[lower].distance_to_cluster(
                    &dictionary,
                    get_point,
                    &centers[j],
                    distance,
        ...
        if inital > max_allowed || found_merge || (enable_phase_3 && measure > overlap_parameter) {
            ...
            large
                .first_mut()
                .unwrap()
                .absorb(&dictionary, get_point, &small[first], distance)?;
...
impl<'b, Z, T: ?Sized> IntermediateCluster<Z, &'b T, T> for MultiCenterRef<'b, T> {
    ...
    fn distance_to_point<'a>(
        ...
        (
            (closest.0 * (1.0 - self.shrinkage as f64) + self.shrinkage as f64 * original.0),
            closest.1,
        )
    }
    ...
    fn absorb<'a>(
         ...
         */
        while representatives.len() > 0
            && self.representatives.len() < self.number_of_representatives
        {
            let mut farthest_weighted_distance = 0.0;
            let mut farthest_index: usize = usize::MAX;
            for j in 0..representatives.len() {
                if representatives[j].1 as f64
                    > (weight as f64) / (2.0 * self.number_of_representatives as f64)
                {
                    let mut new_weighted_distance =
                        (distance)(self.representatives[0].0, representatives[j].0)
                            * representatives[j].1 as f64;
                    check_argument(
                        new_weighted_distance >= 0.0,
                        " weights or distances cannot be negative",
                    )?;
                    for i in 1..self.representatives.len() {
                        let t = (distance)(self.representatives[i].0, representatives[j].0)
                            * representatives[j].1 as f64;
...
impl<T: Clone> MultiCenter<T> {
    ...
    pub fn distance_to_point_and_ref<'a>(
        &'a self,
        point: &T,
        ignore: f32,
        distance: fn(&T, &T) -> f64,
        ...
        Ok((
            (closest.0 * (1.0 - self.shrinkage as f64) + self.shrinkage as f64 * original.0),
            &self.representatives[closest.1].0,
        ))
    }
}
