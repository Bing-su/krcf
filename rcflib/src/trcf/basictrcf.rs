...
impl Bandit {
    ...
    pub fn is_evaluating(&self, arms: usize, internal_timestamp: usize) -> bool {
        arms == self.current_model
            || (internal_timestamp >= self.interval.0 && self.interval.1 >= internal_timestamp)
    }
...
impl State {
    pub fn new(
        ...
        let last_descriptor = Descriptor::new(
            id,
            &Vec::new(),
            0,
            preprocessor.forest_mode() == TIME_AUGMENTED,
            None,
        );
    ...
    U: AugmentedRCF<Label, Attributes>,
{
    let mut result = Descriptor::new(
        ...
        point,
        timestamp,
        state.preprocessor.forest_mode() == TIME_AUGMENTED,
        None,
    );
    ...
    match state.preprocessor.shingled_point(rcf, point, timestamp)? {
        Some(x) => {
            ...
            if state.preprocessor.is_ready() {
                match rcf {
                    Some(y) => state.predictor_corrector.detect_and_modify(
                        &mut result,
                        &state.last_descriptor,
    ...
    state
        .preprocessor
        .post_process(&mut result, point, timestamp, &state.last_descriptor)?;
...
impl BasicTRCF {
    pub fn process(&mut self, point: &[f32], timestamp: u64) -> Result<Descriptor> {
        ...
        match result.rcf_point.as_ref() {
            ...
            Some(x) => {
                ...
                self.rcf.update(
                    &x[(dimension - (dimension / shingle_size))..dimension],
                    timestamp as u64,
                )?;
            }
            _ => {
                self.state.preprocessor.drain(Some(&mut self.rcf))?;
            }
        }
        Ok(result)
    }
    ...
    pub fn process_sequentially(&mut self, input: &[(&[f32], u64)]) -> Result<Vec<Descriptor>> {
        let answer = input
            .into_iter()
            .map(|(a, b)| self.process(*a, *b))
            .collect::<Result<Vec<Descriptor>>>()?
            .into_iter()
            .filter(|x| x.anomaly_grade > 0.0)
            .collect();
        Ok(answer)
    }
    ...
    pub fn extrapolate(
        ...
        self.state
            .preprocessor
            .invert_extrapolation(self.rcf.extrapolate(look_ahead)?)
    }
}
...
impl TRCFOptions {
    pub fn validate(&self, input_dimensions: usize) -> Result<()> {
        check_argument(
            self.transform_decay.unwrap_or(0.0) >= 0.0,
            "transform decay cannot be negative",
        )?;
        // juct check -- the builder should not be modified in case it is reused
        check_argument(
            self.weights
                .as_ref()
                .unwrap_or(&vec![1.0; input_dimensions])
                .len()
                == input_dimensions,
        ...
        check_argument(
            self.default_fill
                .as_ref()
                .unwrap_or(&vec![0.0; input_dimensions])
                .len()
                == input_dimensions,
        ...
        check_argument(
            ...
            "normalization cannot start cannot be after stopping",
        )?;
        Ok(())
    }
}
...
impl BasicTRCFBuilder {
    ...
    pub fn build(&self) -> Result<BasicTRCF> {
        check_argument(
            self.trcf_options.forest_mode != STREAMING_IMPUTE,
            "not yet supported",
        )?;
        ...
        let preprocessor = PreprocessorBuilder::new(self.input_dimensions, self.shingle_size)
            ...
            .stop_normalization(self.trcf_options.stop_normalization)
            .build()?;
        let predictor_corrector = PredictorCorrector::new(
            transform_decay,
            !self.trcf_options.verbose,
        ...
        Ok(BasicTRCF {
            ...
            state: State::new(
                0,
                1,
                self.trcf_options.scoring_strategy,
                predictor_corrector,
                preprocessor,
            )?,
        })
