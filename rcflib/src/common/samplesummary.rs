...
impl SampleSummary {
    ...
    pub fn pick(
        ...
        let mut index = start;
        while index + 1 < weighted_points.len()
            && weighted_points[index].1 as f64 + running < weight
        {
    ...
    where
        Q: ?Sized + Index<usize, Output = f32>,
    {
        check_argument(points.len() > 0, "cannot be empty list")?;
        check_argument(lower_fraction < 0.5, " has to be less than half")?;
        check_argument(upper_fraction > 0.5, "has to be larger than half")?;
...
pub fn summarize(
    ...
    let mut summary =
        SampleSummary::from_points(dimensions, &points, LOWER_FRACTION, UPPER_FRACTION)?;
...
pub fn multi_summarize_ref(
    ...
    let dimensions = points[0].0.len();
    let mut summary =
        SampleSummary::from_references(dimensions, points, LOWER_FRACTION, UPPER_FRACTION)?;
