...
pub fn multi_trcf(transform_method: TransformMethod, parallel_enabled: bool) {
    ...
    let mut multi_trcf: MultiTRCF = MultiTRCFBuilder::new(
        input_dimensions,
        shingle_size,
        number_of_models,
        2 * number_of_series,
    ...
    for y in 0..number_of_series {
        ...
        data_with_key.push(
            multidimdatawithkey::MultiDimDataWithKey::multi_cosine(
                data_size,
                &period,
                &amplitude,
                noise,
    ...
    for i in 0..data_size {
        ...
        for j in 0..number_of_series {
            if next_index[j] < data_with_key[j].change_indices.len()
                && data_with_key[j].change_indices[next_index[j]] == i
            {
                next_index[j] += 1;
                injected.insert(j);
            }
        ...
        let y: usize = result
            .iter()
            .map(|x| {
                if x.relative_index == 0 && injected.contains(&(x.id as usize)) {
                    1
                } else {
                    0
                }
        ...
        let z: usize = result
            .iter()
            .map(|x| {
                if x.relative_index != 0 {
                    let q = (i as i32 + x.relative_index) as usize;
                    if late_discovered[q % shingle_size].contains(&(x.id as usize)) {
                        1
                    } else {
                        0
                    }
    ...
    println!(
        ...
        ((total_overlap as f64) / (total_found as f64)),
        ((total_overlap as f64) / (total_injected as f64))
    );
    println!(
        "with late detection, precision {} recall {}",
        (((total_overlap + late) as f64) / (total_found as f64)),
        (((total_overlap + late) as f64) / (total_injected as f64))
    );
    println!(
        "nontrivial bandit switches {}, affirmations {}",
        multi_trcf.switches(),
        multi_trcf.affirmations()
    );
    ...
    let mut result = multi_trcf
        .states()
        .iter()
        .map(|x| {
            (
                x.bandit.current_model(),
                x.id,
                period_map.get(&x.id).unwrap()[0],
            )
        })
    ...
    for x in 0..number_of_models {
        println!(
            "Model {},  chosen by {}, average period {}, deviation {} ",
            x,
            stat[x].count,
            stat[x].mean(),
            stat[x].deviation()
