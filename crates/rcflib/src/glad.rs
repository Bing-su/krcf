use crate::common::cluster::{multi_cluster_as_weighted_obj, persist, MultiCenter};
...
impl<T: Clone + Sync> GlobalLocalAnomalyDetector<T> {
    ...
    fn compute_weight(&self, random_number: f64, weight: f32) -> f64 {
        f64::ln(-f64::ln(random_number) / weight as f64)
            - ((self.entries_seen - self.most_recent_time_decay_update) as f64 * self.time_decay
    ...
    pub fn score(
        &self,
        current: &T,
        local_distance: fn(&T, &T) -> f64,
        consider_occlusion: bool,
    ) -> Result<Vec<(T, f32)>> {
        if self.clusters.len() == 0 {
            ...
            for j in 0..self.clusters.len() {
                ...
                let close = self.clusters[j].distance_to_point_and_ref(
                    current,
                    self.ignore_below,
                    local_distance,
                )?;
    ...
    pub fn process(
        ...
        if self.sequence_number > self.last_cluster + self.do_not_recluster_within {
            ...
            if f32::abs(current_mean - self.last_mean) > 0.1
                || current_mean > 1.7f32
                || self.sequence_number > self.last_cluster + 20 * self.do_not_recluster_within
            {
                ...
                let temp = multi_cluster_as_weighted_obj(
                    &self.object_list,
                    global_distance,
                    5,
                    0.1,
        ...
        let score: f32 = if score_list.len() == 0 {
            0.0
        } else {
            score_list
                .iter()
        ...
        if score_list.len() > 0 {
            ...
            self.basic_thresholder
                .update_both(score, f32::min(score, other));
        }
