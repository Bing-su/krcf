...
use rcflib::common::cluster::{
    ...
    single_centroid_cluster_weighted_vec,
    single_centroid_cluster_weighted_vec_with_distance_over_slices,
    single_centroid_unweighted_cluster_slice, Center,
};
use rcflib::{common::multidimdatawithkey::MultiDimDataWithKey, l1distance, l2distance};
...
fn test_center(result: &mut Vec<Center>, test_dimension: usize, yard_stick: f32) -> bool {
    ...
    for i in 0..test_dimension {
        result.sort_by(|a, b| {
            a.representative()[i]
                .partial_cmp(&b.representative()[i])
                .unwrap()
        });
...
fn test_config() {
    ...
    let result = single_centroid_cluster_weighted_vec_with_distance_over_slices(
        &input,
        bad_distance,
        2 * test_dimension + 3,
        false,
...
fn core(
    ...
    let mut result = single_centroid_cluster_weighted_vec_with_distance_over_slices(
        &input,
        distance,
        2 * test_dimension + 3,
        false,
...
fn core_as_slice_uniform(
    ...
    let mut result =
        single_centroid_unweighted_cluster_slice(&input, distance, 2 * test_dimension + 3, false)
            .unwrap();
    let answer = (result.len() == 2 * test_dimension)
        && test_center(&mut result, test_dimension, yard_stick);
    println!(" done {} {}", test_dimension, answer);
...
fn core_as_slice_weighted(
    ...
    let mut result = single_centroid_cluster_slice_with_weight_arrays(
        &input,
        &weights,
        distance,
        2 * test_dimension + 3,
...
fn core_vec(
    ...
    let mut result =
        single_centroid_cluster_weighted_vec(&input, distance, 2 * test_dimension + 3, false)
            .unwrap();
    let answer = (result.len() == 2 * test_dimension)
        && test_center(&mut result, test_dimension, yard_stick);
    println!(" done {} {}", test_dimension, answer);
...
fn multi_as_vec(
    ...
    let mut result = multi_cluster_obj(
        &input,
        vec_dist,
        5,
        0.1,
    ...
    for i in 0..test_dimension {
        result.sort_by(|a, b| {
            a.representatives()[0].0[i]
                .partial_cmp(&b.representatives()[0].0[i])
                .unwrap()
        });
        answer = answer && abs(result[0].representatives()[0].0[i] + 2.0 * yard_stick) < 0.5;
        answer = answer
            && abs(result[2 * test_dimension - 1].representatives()[0].0[i] - 2.0 * yard_stick)
                < 0.5;
        for j in 1..(2 * test_dimension - 1) {
...
fn multi_as_ref(
    ...
    let mut result = multi_cluster_as_ref(
        &input,
        vec_dist,
        5,
        0.1,
...
fn multi_as_weighted_ref(
    ...
    let mut result = multi_cluster_as_weighted_ref(
        &input,
        vec_dist,
        5,
        0.1,
...
fn multi_as_vec_weighted(
    ...
    let weights = vec![1.0f32; data_with_key.data.len()];
    let ref_result = multi_cluster_as_object_with_weight_array(
        &input,
        &weights,
        vec_dist,
        5,
