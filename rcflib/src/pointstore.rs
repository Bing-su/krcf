    ...
    <L as TryFrom<usize>>::Error: Debug,
{
    ...
    fn inc_helper(
        ...
        check_argument(
            index < reference_counts.len(),
            "incorrect range of index at insert",
        )?;
    ...
    fn dec_helper(
        ...
        check_argument(
            index < reference_counts.len(),
            "incorrect range of index at delete",
        )?;
...
impl<L, Label, Attributes> PointStore<Label, Attributes>
    for VectorizedPointStore<L, Label, Attributes>
where
    L: Location,
    usize: From<L>,
    Label: Copy + Sync + Send + Into<Attributes>,
    ...
    <L as TryFrom<usize>>::Error: Debug,
{
    fn shingled_point(&self, point: &[f32]) -> Result<Vec<f32>> {
        ...
        if point.len() == base && self.shingle_size > 1 {
            check_argument(
                ...
                "expecting input corresponding to internal shingling",
            )?;
    ...
    fn add(&mut self, point: &[f32], label: Label) -> Result<(usize, usize, Option<Vec<f32>>)> {
        ...
        let attrib_pos = if self.store_attributes {
            ...
            if b_pos == self.attribute_count.len() {
                ...
                Self::inc_helper(
                    b_pos,
                    &mut self.attribute_count,
                    &mut self.hash_attribute_counts,
                )?;
            };
            b_pos
        } else {
        ...
        let label_pos = if self.store_labels {
            ...
            if y >= self.labels.len() {
                ...
                check_argument(
                    self.label_count[y] == 0,
                    " incorrect state in label management",
                )?;
                self.labels.insert(y, (label, attrib_pos));
        ...
        if self.index_manager.is_empty() {
            check_argument(
                self.reference_count.len() == self.location.len(),
                "incorrect state",
            )?;
    ...
    fn inc(&mut self, index: usize, secondary_index: usize) -> Result<()> {
        Self::inc_helper(
            index,
            &mut self.reference_count,
            &mut self.hash_reference_counts,
        )?;
        if self.store_labels {
    ...
    fn dec(&mut self, index: usize, secondary_index: usize) -> Result<()> {
        ...
        if self.store_labels {
            ...
            if secondary_index != usize::MAX {
                ...
                let x = self
                    .attribute_reverse_map
                    .remove(&self.attributes[secondary_index])
                    .expect(" error in secondary");
                check_argument(
                    x == secondary_index,
                    "attribute index accounting is incorrect",
                )?;
            ...
            for i in block_start..block_end {
                ...
                if j_static < end {
                    ...
                    if i == base * locn {
                        ...
                        self.location[new_idx] =
                            (fresh_start / base).try_into().expect("range error");
                        j_static += 1;
                    }
                }
                fresh_start += 1;
