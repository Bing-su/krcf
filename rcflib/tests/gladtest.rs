...
use rcflib::{common::multidimdatawithkey::MultiDimDataWithKey, l2distance};
    ...
    for i in 0..data_size {
        ...
        if rng.gen::<f64>() < 0.005 {
            ...
            data.push(rotate_clockwise(
                &vec,
                (2.0 * PI * i as f32) / data_size as f32
                    + PI * (1.0 + 2.0 * j as f32) / number_of_fans as f32,
            ));
            ...
            data.push(rotate_clockwise(
                &vec,
                (2.0 * PI * i as f32) / data_size as f32
                    + PI * (2.0 * j as f32) / number_of_fans as f32,
            ));
            labels.push(j);
        }
    }
...
fn numeric_glad() {
    ...
    let data_with_key = gen_numeric_data(
        data_size,
        one_seed,
        (5.0 + 1.0 / number_of_fans as f32, 0.0),
        number_of_fans,
    );
    let mut glad =
        GlobalLocalAnomalyDetector::<Vec<f32>>::new(2000, 0, time_decay, 5, 0.1, true).unwrap();
    ...
    for j in 0..data_with_key.data.len() {
        let answer = glad
            .process(&data_with_key.data[j], 1.0, vec_dist, vec_dist, false)
            .unwrap();
        if answer.grade != 0.0 {
    ...
    assert!(glad
        .process(&data_with_key.data[0], -1.0, vec_dist, vec_dist, false)
        .is_err());
    // negative distance is error
    assert!(glad
        .process(&data_with_key.data[0], 1.0, vec_dist, bad_distance, false)
        .is_err());
}
...
pub fn get_ab_array(
    size: usize,
    probability_of_a: f64,
    rng: &mut ChaCha20Rng,
    change_in_middle: bool,
    fraction: f64,
) -> Vec<char> {
    let mut answer = Vec::new();
    let new_size = size + (rng.next_u32() as usize) % (size / 5);
    for i in 0..new_size {
        let toss = if change_in_middle
            && (i as f64 > (1.0 - fraction) * new_size as f64
                || (i as f64) < (new_size as f64) * fraction)
        {
...
fn print_clusters(clusters: &Vec<MultiCenter<Vec<char>>>) {
    for i in 0..clusters.len() {
        println!(
            " Cluster {},  weight {:.3}, average radius {:.3} ",
            i,
            clusters[i].weight(),
            clusters[i].average_radius()
        );
...
fn string_glad() {
    ...
    let mut glad =
        GlobalLocalAnomalyDetector::<Vec<char>>::new(2000, 0, time_decay, 5, 0.1, false).unwrap();
    ...
    for i in 0..data_size {
        ...
        if rng.gen::<f64>() < anomaly_rate {
            ...
            point = get_ab_array(
                ...
                &mut rng,
                flag,
                0.25 * i as f64 / data_size as f64,
            );
        }
