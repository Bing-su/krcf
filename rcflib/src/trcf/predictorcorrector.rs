...
impl PredictorCorrector {
        ...
        U: AugmentedRCF<Label, Attributes>,
    {
        ...
        if base_dimension == 1 {
            ...
            if pick != 0 && pick <= max_attributors {
                ...
                for i in 0..base_dimension {
                    if di_vector.high_low_sum(i + position) >= cutoff
                        && likely_missing_indices.len() < max_attributors
                    {
        ...
        if likely_missing_indices.len() != 0
            && (2 * likely_missing_indices.len() < forest.dimensions())
        {
            let prediction = forest
                .conditional_field(&likely_missing_indices, point, 1.0, false, 0)?
                .median;
                ...
                Some(_x) => {
                    ...
                    if difference < dimensions {
                        ...
                        return differential_remainder > DEFAULT_DIFFERENTIAL_FACTOR * last_score
                            && differential_remainder as f32 * (dimensions as f32)
                                / difference as f32
                                > threshold;
                    } else {
                        return true;
    ...
    pub fn apply_basic_corrector(
        ...
        let last_expected_point = &last_descriptor
            .last_anomaly
            .as_ref()
            .unwrap()
            .expected_rcf_point;
        let last_anomaly_point = &last_descriptor
            .last_anomaly
            .as_ref()
            .unwrap()
            .anomalous_rcf_point;
        let last_relative_index = last_descriptor
            .last_anomaly
            .as_ref()
            .unwrap()
            .relative_index;
        ...
        if last_relative_index == 0 {
            // is is possible to fix other cases, but is more complicated
            if use_difference {
                for y in 0..base_dimension {
                    corrected_point[point.len() - gap * base_dimension + y] += last_anomaly_point
                        [point.len() - base_dimension + y]
                ...
                corrected_point[point.len() - (gap - 1) * base_dimension - 1] +=
                    last_anomaly_point[point.len() - 1] - last_expected_point[point.len() - 1];
            }
        }
        return corrected_point;
    }
    ...
    fn centered_transform_pass(base_dimensions: usize, result: &Descriptor, point: &[f32]) -> f32 {
        ...
        for i in 0..point.len() {
            if absf32(point[i]) * scale[i % base_dimensions]
                > DEFAULT_NORMALIZATION_PRECISION * (1.0 + absf32(shift[i % base_dimensions]))
            {
                max_factor = 1.0;
            }
        }
    ...
    fn calculate_path_deviation(
        ...
        while position + index + base_dimension < point.len() {
            variation += if differenced {
                absf32(point[position + index])
            } else {
                absf32(point[position + index] - point[position + base_dimension + index])
            };
    ...
    fn construct_uncertainty_box(
        ...
        for y in 0..base_dimension {
            ...
            let path_gap = Self::calculate_path_deviation(
                point,
                start_position,
                y,
                base_dimension,
        ...
        U: AugmentedRCF<Label, Attributes>,
    {
        ...
        let total = list.len();
        for e in list {
            if Self::within_unertainty_box(
                uncertainty_box,
                start_position,
                result.scale.as_ref().unwrap(),
                point,
                &e.1,
            ) {
                weight += 1;
    ...
    fn update_score(
        ...
        if result.scoring_strategy == EXPECTED_INVERSE_HEIGHT {
            ...
            self.basic_thresholder
                .update(score, corrected_score, last_score);
        } else {
        ...
        U: AugmentedRCF<Label, Attributes>,
    {
        match &result.rcf_point {
            ...
            Some(point) => {
                ...
                let gap = internal_timestamp
                    - if last_descriptor.last_anomaly.is_none() {
                        0
                    } else {
                        last_descriptor.last_anomaly.as_ref().unwrap().values_seen
                    };
                if score == 0.0 {
                    return Ok(());
                }
                ...
                let (threshold, grade) = if result.scoring_strategy == EXPECTED_INVERSE_HEIGHT {
                    self.basic_thresholder.threshold_and_grade(
                        score,
                        method,
                        dimension,
                        shingle_size,
                ...
                if gap > 0 && gap <= shingle_size && last_descriptor.last_anomaly.is_some() {
                    corrected_score = if result.scoring_strategy == EXPECTED_INVERSE_HEIGHT {
                        ...
                        forest
                            .density_interpolant(&corrected_point)?
                            .distance
                            .total() as f32
                    };
                    let (_, newgrade) = if result.scoring_strategy == EXPECTED_INVERSE_HEIGHT {
                        ...
                        self.basic_thresholder
                            .primary_threshold_and_grade(corrected_score)
                    };
                ...
                let working_grade =
                    grade * Self::centered_transform_pass(base_dimension, result, &corrected_point);
                ...
                let uncertainty_box =
                    Self::construct_uncertainty_box(point, start_position, base_dimension, result);

                if self.auto_adjust
                    && Self::explained_by_conditional_field(
                        &uncertainty_box,
                        point,
                        &corrected_point,
                        start_position,
                ...
                let expected_point = Self::expected_point(
                    &attribution,
                    self.max_attributors,
                    start_position,
                    base_dimension,
                    point,
                    forest,
                ...
                if candidate {
                    if self.auto_adjust {
                        for y in 0..base_dimension {
                            self.deviations_actual[y]
                                .update(point[dimension - base_dimension + y] as f64);
                            self.deviations_expected[y]
                                .update(expected_point[dimension - base_dimension + y] as f64);
                        }
                        if self.run_length > DEFAULT_RUN_ALLOWED {
                            let mut within = true;
                            for y in 0..base_dimension {
                                within = absf32(
                                    self.deviations_actual[y].mean() as f32
                                        - point[dimension - base_dimension + y],
                                ) < maxf32(
                                    2.0 * self.deviations_actual[y].deviation() as f32,
                                    NOISE_FACTOR
                                        * result.difference_deviations.as_ref().unwrap()[y],
                                );
                                within = within
                                    && absf32(
                                        self.deviations_expected[y].mean() as f32
                                            - expected_point[dimension - base_dimension + y],
                                    ) < 2.0
                                        * maxf32(
                                            self.deviations_expected[y].deviation() as f32,
                                            self.deviations_actual[y].deviation() as f32,
                                        )
                                        + 0.1
                                            * absf32(
                                                (self.deviations_actual[y].mean()
                                                    - self.deviations_expected[y].mean())
                                                    as f32,
                                            );
                            }
                            if within {
