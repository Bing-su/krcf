    ...
    <N as TryFrom<usize>>::Error: Debug,
{
    ...
    >(
        ...
        if self.root == self.node_store.null_node() {
            ...
            if self.store_attributes {
                self.node_store
                    .add_attrib_at_leaf(point_index, point_attribute)?;
            }
            Ok(point_index)
        } else {
            ...
            if point.eq(old_point) {
                ...
                if self.store_attributes {
                    self.node_store
                        .add_attrib_at_leaf(leaf_point_index, point_attribute)?;
                }
                ...
                if self.store_pointsum {
                    self.node_store
                        .recompute_pointsum(merged_node, point_store)?;
                }
                if self.propagate_attributes {
                    self.node_store
                        .recompute_attribute_vec(merged_node, point_store)?;
                }
                if saved_parent != self.node_store.null_node() {
                    self.node_store.manage_ancestors_add(
                        &mut path_to_root,
    ...
    >(
        ...
        if self.store_attributes {
            self.node_store
                .del_attrib_at_leaf(leaf_point_index, point_attribute)?;
        }
    ...
    >(
        ...
        let (cut_dimension, cut_value, _left_child, _right_child) =
            self.node_store.cut_and_children(self.root);
        let mass = self.node_store.mass(self.root);
        let mut node_view =
            MediumNodeView::new::<Label, Attributes>(self.root, cut_dimension, cut_value, mass);
